...
import (
	...
	"github.com/matisiekpl/unipdf/v3/internal/cmap"
...
func parseCIDFontWidthsArray(w core.PdfObject) (map[textencoding.CharCode]float64, error) {
	...
	for i := 0; i < wArrLen-1; i++ {
		...
		case *core.PdfObjectArray:
			...
			for j := 0; j < arr.Len(); j++ {
				width, err := core.GetNumberAsFloat(core.TraceToDirectObject(arr.Get(j)))
				if err != nil {
					return nil, fmt.Errorf("Bad font W array obj1: i=%d %#v", i, obj1)
				}
				fontWidths[textencoding.CharCode(n+j)] = width
			}
